...
각 서비스 포트는 동일하게 SSH 데몬에서 관리하므로 22다. 이와 같은 기본 프로토콜의 포트는 항상 기본값이 정해져있으므로, 대부분의 공격 시도는 해당 프로토콜의 기본 서비스 포트를 통해 이루어진다. 반대로 말하자면, 서비스 포트를 변경하는 것만으로도 공격의 대부분을 차단할 수 있다.

SSH의 22라는 기본 포트와 달리, 서버 담당자가 임의로 바꾼 포트는 추적할 방법이 없기 때문. 포트의 최대값은 65535개이므로, 임의의 포트로 변경했다면 공격자는 모든 포트에 통신을 시도하여 SSH 포트를 찾아내야하는 전처리 작업이 필요하게된다.
...
`iptables`를 활용하면 특정 IP만 해당 포트에 접근할 수 있도록 제한할 수도 있다. geoIP 플러그인과 연동하면 국가별 IP 대역별로 관리할 수도 있으니, 관심이 있다면 찾아보길 바란다. 적용이 마냥 간단하지는 않으니 참고할 것.
...
위 명령어를 입력하여 키 파일을 등록하자. `/home/username/.ssh/id_rsa.pub`를 자동으로 등록한다.
