...
이와 같이, 각 자릿수 별로 나올 수 있는 선분의 합을 가지는 숫자의 최소값을 식으로 도출한다면 <span class="primary">다이나믹 프로그래밍</span>을 적용하여 `dp[i][j]`의 형태로 다룰 수 있다.

* $i$: 자릿수 $(i = 1, 2, \, \, \, \dotsm \, \, \, , 14, 15)$
* $j$: 선분의 합

만약 $dp[3][6]$을 구할 경우, 세 자릿수의 선분 합이 6인 숫자의 최소값이므로 111(2 + 2 + 2)가 된다. 이렇게 적절한 값이 나올 수 있도록 `dp` 배열의 표현식을 도출해야한다.
...
값이 크므로 `long` 배열로 선언한다.

$N = 3$일 경우, 세 자릿수를 가지며, 선분 합의 범위는 6 ~ 21이다. `new int[3][21]`일 경우, $i$는 0 ~ 2의 값을 가지게 된다. 
