...
import { getFormattedDate, slugRegex } from "../../common/common";
...
export default function Post({ page, post, group })
{
	...
	const slugs = slugRegex.exec(post.slug);
...
export async function getStaticProps({ params })
{
	...
	const post = getContentBySlug("posts", params.slug.join("-"));
...
export async function getStaticPaths()
{
	...
	return {
		...
		{
			const arr = slugRegex.exec(post.slug);

			return {
				params: {
					slug: [ arr[1], arr[2], arr[3], arr[4] ]
				}
			};
		}),
		fallback: false
