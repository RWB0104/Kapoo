...
규모를 작게 보자면 학창 시절에서도 단위로 인한 문제는 흔히 볼 수 있었다. 이과생들이라면 수학이나 과학 시험을 볼 때, 값은 맞았음에도 단위를 표기하지 않거나, 오입하여 오답 처리가 된 뼈아픈 경험이 있을 것이다.

<br />

여기서 우리는 단위의 중요성을 알 수 있다. 단위 하나로 값. 즉, 데이터의 의미가 완전히 다르게 해석될 수 있기 때문이다.

공간정보 좌표계라는 단위를 가지고 있다. 공간정보에는 수 많은 좌표계가 있으며, 좌표계를 잘 못 대입할 경우 전혀 다른 위치가 계산된다.

즉, 좌표계가 잘못 입력되면 네비게이션이 엉뚱한 곳을 가리킬수도, 미사일이 엄한 위치로 타격될 수도 있는 것이다. 때문에 좌표계는 공간정보에서 매우 중요한 요소 중 하나다.

대부분 떠오르는 동경 127, 북위 37 같은 좌표도 수 많은 좌표계 중 하나다. 동경 127, 북위 37은 GPS에서 주로 사용하는 세계측지 중 하나인 `WGS-84` 좌표계다. 이와 같이 좌표계의 특성에 따라 표현되는 값도 다르며, 이를 구분하기 위해 EPSG:0000과 같이 EPSG 코드로 관리한다.
